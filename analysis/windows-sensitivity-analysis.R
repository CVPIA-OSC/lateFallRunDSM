...
run_scenario <- function(scenario, sensi_params) {
  seeds <- late_fall_run_model(mode = "seed", ..params = sensi_params, stochastic = FALSE)
  run <- late_fall_run_model(scenario = scenario,
...
clusterExport(cl, list('run_scenario', 'late_fall_run_model', 'scenarios'))
...
param_sensitivity <- function(param) {
  scalars <- if (param == "weeks_flooded") {
    seq(0, 4, by = 1)
  } else if (param %in% c("juveniles_at_chipps_model_weights", "adults_in_ocean_model_weights")) {
    list(c(0.5,0,0.5),
         c(0.5,0.5,0),
         c(1,0,0),
         rep(1/3,3),
